...
def run_custom(g: ig.Graph, out_dir: str, trial_idx: int):
    """
    Run custom infomap implementation on the input graph, write result to json
    & return results.
    """
    path = f"{out_dir}/custom_trial{trial_idx}.json"
...
def run_igraph(g: ig.Graph, out_dir: str, trial_idx: int):
    """
    Run igraph package's infomap on the input graph, write result to json
    & return results.
    """
    path = f"{out_dir}/igraph_trial{trial_idx}.json"

    t0 = time.perf_counter()
    clustering = g.community_infomap(trials=1)
    t_elapsed = time.perf_counter() - t0
...
def run_infomap_pkg(g: ig.Graph, directed: bool, out_dir: str, trial_idx: int):
    """
    Run infomap package's infomap on the input graph, write result to json
    & return results.
    """
    path = f"{out_dir}/infomap_pkg_trial{trial_idx}.json"  # single file: this method's one run covers all trials
...
def run_variant(directed: bool, num_trials=NUM_TRIALS):
    ...
    for idx in range(NUM_TRIALS):
        print(f"Trial {idx+1}/{NUM_TRIALS}")

        # run igraph
        print("Running igraph infomap ...")
        res_ig = run_igraph(g, dir, trial_idx=idx)
        ...
        res_im = run_infomap_pkg(g, directed, dir, trial_idx=idx)
        ...
        res_custom = run_custom(g, dir, trial_idx=idx)
